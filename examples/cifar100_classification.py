...
class MaxAbsNorm(nn.Module):
    def forward(self, x):
        max_abs = torch.max(torch.abs(x), dim=1, keepdim=True)[
            0
        ]  # Get max per feature map
        return x / (max_abs + 1e-8)  # Avoid division by zero


class AdaptiveConvNet(nn.Module):
    def __init__(self, num_points=3, position_init="random", channels:int=16):
        super().__init__()
        ...
        # self.max_abs = MaxAbsNorm()
        self.max_abs = noop

        # Calculate the size of flattened features
        # Input: 32x32 -> Conv1: 30x30 -> Pool1: 15x15
        # Conv2: 13x13 -> Pool2: 6x6
        # Conv3: 4x4 -> Pool3: 2x2 -> Flattened: 2*2*128
        self.fc1 = nn.Linear(2 * 2 * self.layer_3_channels, 100)

    def forward(self, x):
        x = self.pool1(self.max_abs(self.conv1(x)))
        x = self.pool2(self.max_abs(self.conv2(x)))
        x = self.pool3(self.max_abs(self.conv3(x)))
        x = x.reshape(-1, 2 * 2 * self.layer_3_channels)
        x = self.fc1(x)
        return x
    ...
    def move_smoothest(self, weighted: bool = True):
        success = True
        with torch.no_grad():
            self.conv1.move_smoothest(weighted=weighted)
            self.conv2.move_smoothest(weighted=weighted)
            self.conv3.move_smoothest(weighted=weighted)
        return success
...
class HybridConvNet(nn.Module):
    def __init__(self, num_points=3):
        ...
        # Conv2: 13x13 -> Pool2: 6x6
        # Conv3: 4x4 -> Pool3: 2x2 -> Flattened: 2*2*128
        self.fc1 = AdaptivePiecewiseLinear(2 * 2 * 128, 100, num_points=num_points)
...
class StandardConvNet(nn.Module):
    def __init__(
    ...
    ):  # num_points is ignored but kept for consistent interface
        super().__init__()
        # First convolutional layer: 3 input channels (RGB), 32 output channels
        self.conv1 = nn.Conv2d(3, 32, kernel_size=3)
        self.pool1 = nn.MaxPool2d(2)
        ...
        # Conv2: 13x13 -> Pool2: 6x6
        # Conv3: 4x4 -> Pool3: 2x2 -> Flattened: 2*2*128
        self.fc1 = nn.Linear(2 * 2 * 128, 100)
...
def train(
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in tqdm.tqdm(enumerate(train_loader)):
            ...
            if writer is not None and batch_idx % log_interval == 0:
                ...
                writer.add_scalar(
                    "training/batch_accuracy", batch_accuracy, global_step
                )
                global_step += 1

            if training_step%adapt_frequency==0:
                if adapt == "move":
                    model.move_smoothest(weighted=True)
                    optimizer = generate_optimizer(model.parameters(), learning_rate)
                    # print('adapting')
                elif adapt == "reset":
                    # only reset the optimizer
                    optimizer = generate_optimizer(model.parameters(), learning_rate)
                elif adapt is None or adapt == "null":
                    pass
                else :
                    raise ValueError(f"Option adapt={adapt} not valid choose null, move, reset")
    ...
    return (
        ...
        final_train_accuracy if epochs > 0 else 0,
    )
...
def plot_results(train_losses, test_accuracies, save_dir, val_accuracies=None):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "cifar100_training_results.png"))
    plt.close()


@hydra.main(
    version_base=None, config_path="config", config_name="cifar100_classification"
)
def main(cfg: DictConfig):
    """Train an adaptive convolutional network on CIFAR-100"""
    ...
    # Store CIFAR-100 data in the project root directory to avoid re-downloading
    data_dir = os.path.join(project_root, "data")
    logger.info(f"Using CIFAR-100 data directory: {data_dir}")
    ...
    logger.info(
        f"Split training data: {len(train_subset)} training samples, {len(val_subset)} validation samples"
    )
    ...
    if training_fraction < 1.0:
        ...
        logger.info(
            f"Using {num_train_samples} training samples ({training_fraction:.2%} of the training subset)"
        )
    ...
    if model_type == "adaptive":
        model = AdaptiveConvNet(num_points=num_points, channels=channels).to(device)
        logger.info(
            f"AdaptiveConvNet initialized with {num_points} points per piecewise function"
        )
    elif model_type == "hybrid":
        model = HybridConvNet(num_points=num_points).to(device)
        logger.info(
            f"HybridConvNet initialized with {num_points} points in the linear layer"
        )
        ...
        logger.error(
            f"Unknown model type: {model_type}. Using AdaptiveConvNet as default."
        )
